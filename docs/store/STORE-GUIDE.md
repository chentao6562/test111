...
套餐有两种库存策略，门店员工需了解其差异：

| 策略 | 说明 | 库存计算 | 备货影响 |
|------|------|---------|---------|
| COMPONENT（按组成商品计算） | 套餐库存根据组成商品动态计算 | 各商品可组成套餐的最小数量 | 核销扣减各组成商品库存 |
| INDEPENDENT（独立库存） | 套餐有独立库存，与单品分开 | 套餐自身的独立库存 | 核销只扣减套餐库存 |

**COMPONENT策略示例**：
- 套餐A包含：商品甲(×2) + 商品乙(×1)
- 当前库存：商品甲=10，商品乙=3
- 套餐可售数量：min(10÷2, 3÷1) = min(5, 3) = **3套**
- 核销1套后：商品甲=8，商品乙=2

**INDEPENDENT策略示例**：
- 套餐B独立库存：5套
- 核销1套后：套餐B库存=4套（不影响组成商品库存）
