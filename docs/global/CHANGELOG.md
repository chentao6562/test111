...
### 最近更新（完整记录）
- [2026-01-25](#2026-01-25-套餐功能上线) - **套餐功能：多商品组合销售、三种定位、两种库存策略**
...
### 功能概述
套餐是将多个单品组合销售的商品形式，提供一站式购物体验，提升客单价和毛利率。
...
| 策略 | 编码 | 说明 |
|------|------|------|
| 按组成商品计算 | COMPONENT | 库存=各商品可组成套餐的最小数量，核销扣减组成商品库存 |
| 独立库存 | INDEPENDENT | 套餐有独立库存，不影响单品库存 |

#### 四级价格体系
套餐同样遵循系统的四级价格结构：成本价 ≤ 供货价 ≤ subPrice ≤ 零售价

### 数据库新增（3表）
| 表名 | 说明 |
|------|------|
| ProductPackage | 套餐主表（名称、定位、定价、库存策略） |
| PackageItem | 套餐商品明细表（商品ID、数量、价格快照） |
| PackageAgentPrice | 套餐推销员定价表（推销员ID、零售价、subPrice） |
...
| PUT | /api/packages/:id/price | 推销员设置套餐价格 |
| POST | /api/packages/:id/reserve | 创建套餐预约 |
...
server/src/routes/package.ts                  # H5端路由
server/src/routes/adminPackage.ts             # 管理后台路由
h5-agent/src/views/packages/                  # 套餐前端页面
admin/src/views/package/                      # 套餐管理页面
```

### 特殊规则
- **不参与营销活动**：套餐不参与砍价、拼团、锁价、秒杀等活动
- **推销员定价**：一级需设置subPrice后，二级才能销售
