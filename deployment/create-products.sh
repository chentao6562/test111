...
curl -s -X POST "$API" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d '{
  ...
  "transferFee": "35",
  "allowTransfer": true
}' > /dev/null && echo "✓ 龙腾盛世礼花组合"

# 分类2: 礼花弹（假设categoryId=2）
echo ""
echo "创建礼花弹..."
...
curl -s -X POST "$API" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d '{
  ...
  "transferFee": "60",
  "allowTransfer": true
}' > /dev/null && echo "✓ 彩虹天际8发礼花弹"

# 分类3: 儿童烟花（假设categoryId=3）
echo ""
echo "创建儿童烟花..."
...
curl -s -X POST "$API" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d '{
  ...
  "transferFee": "0",
  "allowTransfer": false
}' > /dev/null && echo "✓ 快乐童年旋转小蜜蜂"

# 分类4: 家用套装（假设categoryId=4，需要先创建）
echo ""
echo "创建家用套装..."
...
curl -s -X POST "$API" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d '{
  ...
  "images": ["uploads/products/礼花.jpg", "uploads/products/u=617237994,2911142058&fm=253&app=138&f=JPEG.jpg"],
  "retailPrice": "388",
  "agentPrice": "268",
  "wholesalePrice": "188",
  "stock": 250,
...
curl -s -X POST "$API" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d '{
  ...
  "transferFee": "40",
  "allowTransfer": true
}' > /dev/null && echo "✓ 庭院烟花盛宴套装"

# 分类5: 商用礼炮（categoryId=2）
echo ""
echo "创建商用礼炮..."
...
curl -s -X POST "$API" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d '{
  ...
  "transferFee": "120",
  "allowTransfer": true
}' > /dev/null && echo "✓ 盛大开业288响礼炮"

echo ""
echo "所有商品创建完成！共10个商品。"
