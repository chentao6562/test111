...
const TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MSwidHlwZSI6ImFkbWluIiwicm9sZSI6IkFETUlOIiwiaWF0IjoxNzY4MjIxMzQxLCJleHAiOjE3Njg4MjYxNDF9.5KwS8CEXUUX9bK1Iae-F0vGmcl8lD8U0bU7GJdcdD3c";
const API = "http://39.104.58.26/api/admin/products";
...
async function createProducts() {
  ...
  for (let i = 0; i < products.length; i++) {
    try {
      const response = await axios.post(API, products[i], {
        headers: {
          'Authorization': `Bearer ${TOKEN}`,
          'Content-Type': 'application/json; charset=utf-8'
        }
      });
      console.log(`✓ [${i+1}/10] 创建成功: ${products[i].name}`);
    } catch (error) {
      console.error(`✗ [${i+1}/10] 创建失败: ${products[i].name}`);
