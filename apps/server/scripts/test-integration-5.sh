...
echo "=========================================="
echo "穿行测试5 - BE-01~10 联合测试"
echo "=========================================="

# 获取Token
USER_RESP=$(curl -s -X POST "${BASE_URL}/auth/uni-login" -H "Content-Type: application/json" -d '{"token":"integration5_user"}')
...
echo ""
echo "=========================================="
echo "穿行测试5结果: $PASSED/$TOTAL 通过"
echo "=========================================="

if [ $FAILED -eq 0 ]; then
  echo -e "${GREEN}穿行测试5通过!${NC}"
  exit 0
else
