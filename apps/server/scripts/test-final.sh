...
test_api() {
  ...
  if [ "$code" == "0" ] || [ "$http_code" == "$expected_code" ]; then
    ...
    echo -e "${GREEN}[PASS]${NC} $name"
    PASSED=$((PASSED + 1))
  elif [ "$expected_code" == "400" ] && ([ "$http_code" == "400" ] || [ "$code" != "0" ]); then
    echo -e "${GREEN}[PASS]${NC} $name"
    PASSED=$((PASSED + 1))
...
echo "=========================================="
echo -e "${YELLOW}最终完整测试 - BE-01~15 所有模块${NC}"
echo "=========================================="
echo ""

# 获取Token
echo "正在获取测试Token..."
USER_RESP=$(curl -s -X POST "${BASE_URL}/auth/uni-login" -H "Content-Type: application/json" -d '{"token":"final_test_user"}')
...
test_api "获取用户信息" "GET" "${BASE_URL}/auth/profile" "" "$USER_TOKEN" "200"
test_api "无Token访问" "GET" "${BASE_URL}/auth/profile" "" "" "401"
...
test_api "商品列表(分页)" "GET" "${BASE_URL}/product/list?page=1&pageSize=10" "" "$USER_TOKEN" "200"
test_api "分类列表" "GET" "${BASE_URL}/product/category" "" "$USER_TOKEN" "200"

# ==================== BE-05 订单模块 ====================
echo "--- BE-05 订单模块 ---"
test_api "订单列表" "GET" "${BASE_URL}/order/list" "" "$USER_TOKEN" "200"
test_api "订单列表(分页)" "GET" "${BASE_URL}/order/list?page=1&pageSize=10" "" "$USER_TOKEN" "200"
...
# ==================== 权限测试 ====================
echo "--- 权限测试 ---"
test_api "无Token访问用户信息" "GET" "${BASE_URL}/user/info" "" "" "401"
test_api "无Token访问订单" "GET" "${BASE_URL}/order/list" "" "" "401"
test_api "无Token访问管理后台" "GET" "${BASE_URL}/admin/dashboard/stats" "" "" "401"

echo ""
echo "=========================================="
echo -e "最终完整测试结果: ${GREEN}$PASSED${NC}/${TOTAL} 通过"
echo "=========================================="

if [ $FAILED -eq 0 ]; then
  echo -e "${GREEN}✓ 最终完整测试通过!${NC}"
  exit 0
else
